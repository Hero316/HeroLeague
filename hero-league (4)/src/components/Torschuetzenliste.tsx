...
export default function Torschuetzenliste({ matches, teams }: TorschuetzenlisteProps) {
  // Calculate scorer stats based on the "scorers" array of completed/live matches
  const scorerCounts = React.useMemo(() => {
    const counts: { [playerName: string]: { name: string; teamName: string; teamLogoColor: string; logoIcon: string; logoUrl: string; goals: number } } = {};

    matches.forEach(m => {
      const isCompletedOrLive = m.status === 'beendet' || m.isCompleted || m.status === 'live';
      if (isCompletedOrLive && m.scorers) {
        m.scorers.forEach(s => {
          if (!s.playerName || s.playerName === 'Eigentor' || s.playerName === 'Unbekannt') return; // skip own goals/unknown

          const team = teams.find(t => t.id === s.teamId);
          const teamName = team ? team.name : 'Unbekannt';
          const teamLogoColor = team ? team.logoColor : '#3B82F6';
          const logoIcon = team ? team.logoIcon : '⚽';
              ...
              >
                <div className="flex items-center space-x-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-sans font-semibold text-white text-sm sm:text-base">
                      {scorer.name}
                    </h4>
