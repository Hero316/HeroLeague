...
export default function Statistiken({ players, matches, teams }: StatistikenProps) {
  ...
  const clubStats = React.useMemo(() => {
    ...
    // Compute
    matches.forEach(m => {
      const isFinished = m.status === 'beendet';
            ...
            {topScorers.slice(0, 5).map((player, idx) => (
              ...
              >
                <div className="flex items-center space-x-3 min-w-0">
                  ...
                  <PlayerAvatar name={player.name} imageUrl={player.imageUrl} color={player.teamLogoColor} size="sm" />
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-[10px] text-gray-500 font-mono">
                    ...
                    {player.matchesPlayed > 0 && ` (${(player.goals / player.matchesPlayed).toFixed(1)}/Sp)`}
                  </div>
                </div>
              </div>
            ))}
