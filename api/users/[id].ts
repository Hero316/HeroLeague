...
import { getSession, requireSuperadmin } from '../_lib/auth.js';
...
const updateUser = requireSuperadmin(async (req: VercelRequest, res: VercelResponse) => {
  ...
  const rows = await sql`SELECT id, email, name, role, is_active AS "isActive" FROM users WHERE id = ${id}`;
  if (rows.length === 0) return res.status(404).json({ error: 'Benutzer nicht gefunden.' });
  const user = rows[0] as { id: string; email: string; name: string; role: string; isActive: boolean };
  ...
  await sql`
    UPDATE users SET name = ${nextName}, role = ${nextRole}, is_active = ${nextActive}
    WHERE id = ${id}
  `;
  return res.json({ id, email: user.email, name: nextName, role: nextRole, isActive: nextActive });
});
