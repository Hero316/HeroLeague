...
export const ACTION_CATALOG: ActionDef[] = [
  ...
  { key: 'goal_header', label: 'Kopfballtor', hint: 'Tor mit dem KOPF: "köpft ein", "Kopfball ins Netz", "per Kopf", "Kopfballtor". Nur goal_header ausgeben – die App zählt es automatisch als normales Tor (goal) mit.' },
...
1. Ordne jede erkennbare Aktion GENAU einem Spieler aus den obigen Kadern zu. Nutze im Feld "player" den EXAKTEN Namen aus dem Kader. Der Sprecher nennt oft nur Vorname, Spitzname ODER die Rückennummer ("die Nummer 5", "die 7") – ordne über die #Nummer bzw. den Namen dem richtigen Kaderspieler zu und gib trotzdem den EXAKTEN Namen aus. Im Feld "team" den passenden Team-Namen ("${ctx.homeTeam}" oder "${ctx.awayTeam}").
   WICHTIG – Spieler NICHT im Kader: Wird ein Spieler klar mit Namen genannt, steht aber NICHT im obigen Kader, dann ERFINDE keinen Kadernamen und ordne ihn NICHT einem falschen Kaderspieler zu. Gib stattdessen den gehörten Namen wörtlich im Feld "player" aus, "team" = die genannte/erkennbare Mannschaft, und setze eine kurze "note" wie "(nicht im Kader)". So kann er nachträglich angelegt werden. (Nur bei reinen Rückennummern ohne Namen, die zu keinem Kaderspieler passen: weglassen.)
