...
export default function Statistiken({ players, matches, teams }: StatistikenProps) {
  // Sort players for Top Scorers
  const topScorers = [...players].sort((a, b) => b.goals - a.goals);
  
  // Sort players for Top Assists
  const topAssists = [...players].sort((a, b) => b.assists - a.assists);

  // Dynamic club-wide calculations
  const clubStats = React.useMemo(() => {
    const stats: { [teamId: string]: { goalsFor: number; goalsAgainst: number; name: string; logoColor: string; logoIcon: string } } = {};
    
    // Initialize
    teams.forEach(t => {
      stats[t.id] = { goalsFor: 0, goalsAgainst: 0, name: t.name, logoColor: t.logoColor, logoIcon: t.logoIcon };
    });
    ...
    const bestAttack = [...list].sort((a, b) => b.goalsFor - a.goalsFor)[0];
    
    // Best defense (fewest goals conceded, of teams that have played at least 1 game)
    const bestDefense = [...list].sort((a, b) => a.goalsAgainst - b.goalsAgainst)[0];
